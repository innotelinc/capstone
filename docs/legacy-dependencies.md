...
| SigNoz v0.138 (unified binary + separate ClickHouse Keeper, 2024-era) | Old release line; the `signoz` unified binary was a transition build. ClickHouse bumped to 25.4. | Current SigNoz 0.4x release line (own compose topology from upstream) | ClickHouse bumped to 25.4. SigNoz 0.4x topology migration remains (bump `signoz`/`signoz-otel-collector`/`alertmanager` together, re-verify OTel ingest in `otel-collector-config.yaml`). |
...
| ClickHouse 25.12.5 pinned with hand-written `clickhouse-config.yaml` / `clickhouse-keeper.yaml` | ~~Single-node cluster config written by hand~~ **Bumped to 25.4.** Still hand-written; SigNoz's own topology is the long-term target. | SigNoz's own compose/helm values (or a managed ClickHouse when multi-node) | ClickHouse bumped to 25.4; SigNoz topology migration remains. |
