...
# The Stasis application name is GENERATED by dograh per telephony config
# (dograh_<hex>) and persisted as DOGRAH_STASIS_APP_NAME by scripts/dograh_wire.py.
# Dialplan that calls a bare "dograh" would name an app no ARI client has
# registered, so Asterisk falls straight through to Hangup() and the call dies.
STASIS_APP_PLACEHOLDER = "dograh"


def stasis_app_name(explicit: str | None = None) -> str:
    """The ARI app Asterisk must route into (env → explicit → legacy default)."""
    return (explicit or "").strip() or os.environ.get("DOGRAH_STASIS_APP_NAME", "").strip() \
        or STASIS_APP_PLACEHOLDER
...
class DograhClient:
    ...
    def __init__(
        ...
        self._stasis_app: str | None = None
    ...
    def discovered_stasis_app_name(self) -> str:
        """The ARI app Asterisk must route into, discovered from dograh itself.

        dograh generates ``dograh_<hex>`` per telephony config and exposes it
        as ``credentials.stasis_app_name`` on the config detail endpoint — the
        same field ``scripts/dograh_wire.py`` persists as
        ``DOGRAH_STASIS_APP_NAME``. Preferring the env var keeps the common
        path call-free, while the live lookup means a missing/stale env still
        yields dialplan that calls the app an ARI client actually registered
        (a bare ``dograh`` falls straight through to Hangup()).
        """
        if self._stasis_app is not None:
            return self._stasis_app
        explicit = os.environ.get("DOGRAH_STASIS_APP_NAME", "").strip()
        if explicit:
            self._stasis_app = explicit
            return self._stasis_app
        name = ""
        try:
            ...
            name = str(
                ((detail or {}).get("credentials") or {}).get("stasis_app_name") or ""
            ).strip()
        except DograhError:
            name = ""
