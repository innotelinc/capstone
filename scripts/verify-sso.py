#!/usr/bin/env python3
"""verify-sso.py — prove Capstone's sign-in posture still holds on a live box.

Three things are asserted, because all three are load-bearing:

  1. Every public name demands Cerulean Authentik. Each oauth2-proxy gateway is
     driven through a real authorization-code flow with a temporary Authentik
     identity, and the sealed session (`_innotel_sso`) must then open the app.
     This is what catches a gateway whose redirect_uri was never registered, or
     whose client secret was rotated without re-deploying it.
  2. The group check is real. The same flow with an identity that is *not* in
     SSO_REQUIRED_GROUP must be refused. A gateway that admits every identity
...
Config (environment, falling back to this repo's .env):
    ...
    SSO_AUTHENTIK_BASE          IdP origin; also where the API is, unless
    AUTHENTIK_API_URL           ...is set explicitly
    AUTHENTIK_TOKEN             Authentik API token (admin). Required;
    AUTHENTIK_BOOTSTRAP_TOKEN   the same value under its Cerulean name
...
Usage:
    ...
    python3 scripts/verify-sso.py --base capstone.innotel.us --verbose
"""
...
MEMBER_USER = "e2e-capstone-sso"
OUTSIDER_USER = "e2e-capstone-outsider"
SESSION_COOKIE = "_innotel_sso"

# Public names this zone owns, in the order it is worth checking them. Every one
# is fronted by an oauth2-proxy gateway, and every gateway is a client of the
# same Authentik application (SSO_APP_SLUG).
SUBDOMAINS = [
    ("n8n", "n8n.{base}"),
    ("grist", "grist.{base}"),
...
# (label, port) — bound to 127.0.0.1 only, because the apps behind them trust the
# proxy for identity. A LAN answer here is the bypass this test exists for.
LAN_ONLY_PORTS = [
    ("n8n", 5678),
    ("grist", 8484),
    ("grafana", 3301),
...
class Config:
    def __init__(self, args):
        ...
        self.base = (args.base or pick("CAPSTONE_SSO_BASE", "NPM_BASE_DOMAIN",
                                       default="capstone.innotel.us")).strip("/")
        self.idp = pick("SSO_AUTHENTIK_BASE", "AUTHENTIK_API_URL",
                        default="https://auth.cerulean.innotel.us").rstrip("/")
        self.api = pick("AUTHENTIK_API_URL", default=self.idp).rstrip("/") + "/api/v3"
        self.token = pick("AUTHENTIK_TOKEN", "AUTHENTIK_BOOTSTRAP_TOKEN")
        ...
        if not self.token:
            raise CannotRun(
                "no Authentik API token: set AUTHENTIK_TOKEN or "
                "AUTHENTIK_BOOTSTRAP_TOKEN (env or .env)"
            )
        if not self.lan_ip:
...
def main():
    ...
    api = AuthApi(cfg)
    print("Capstone SSO verification")
    print(f"  idp     : {cfg.idp}")
    print(f"  api     : {cfg.api}")
    ...
    try:
        ...
        member_pk = api.make_user(MEMBER_USER, "Capstone SSO Verification",
                                  [group_pk] if group_pk else [])
        outsider_pk = api.make_user(OUTSIDER_USER, "Capstone SSO Outsider")
