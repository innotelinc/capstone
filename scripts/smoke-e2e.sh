...
#     • regression guard: Stasis() runs BEFORE the voicemail-fallback GotoIf
#       (a GotoIf placed before Stasis always jumps — mailbox unset → hangup —
#       leaving the agent unreachable; every inbound call cleared instantly)
#
#   Media WebSocket
...
if echo "$dp" | grep -q "'8000'"; then
  ...
  if echo "$dp" | grep -q "Stasis(dograh"; then
    ...
    # no dograh agent could ever answer. Compare line positions inside the
    # exten-8000 block only — other extens' lines must not influence it.
    block=$(echo "$dp" | sed -n "/'8000' =>/,/'8001' =>/p")
    ...
    if [[ -z "$gotoif_line" ]]; then
      pass "no vm-fallback GotoIf on exten 8000 — nothing to guard"
    elif [[ -n "$stasis_line" && "$stasis_line" -lt "$gotoif_line" ]]; then
      pass "regression guard: exten 8000 runs Stasis before the vm-fallback GotoIf"
    else
      fail "regression: vm-fallback GotoIf runs before Stasis on exten 8000 — calls hang up before the agent answers (fix pbx/asterisk/extensions_custom.conf)"
    fi
  else
